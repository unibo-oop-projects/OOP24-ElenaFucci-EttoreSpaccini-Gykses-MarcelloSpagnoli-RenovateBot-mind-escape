 ...
 * <p>
 * This factory provides methods to create various types of interactable objects
 * in the game world, such as pickable items, doors (locked with enigmas or items),
 * and unpickable objects with specific interaction behaviors.
 * </p>
 *
...
public class InteractableFactoryImpl implements InteractableFactory {
    ...
    public Pickable createPickable(final String name, final Optional<Point2D> position,
        ...
        return new PickableImpl(position, name, dimensions, description);
    }
     ...
     * @param pickable   the item required to unlock the door
     * @param rooms      the pair of rooms connected by the door
    ...
    public Door createDoorLockedWithPickable(final String name, final Optional<Point2D> position,
                                             final Dimensions dimensions, final Pickable pickable,
                                             final Pair<Room, Room> rooms) {
        return new DoorLockedWithPickable(
            new DoorImpl(position, name, dimensions, rooms),
            pickable
        );
    }
     ...
     * optionally providing a reward upon successful resolution.
     *
     ...
     * rewarding the player with another pickable item after unlocking.
     *
     * @param name       the name of the unpickable object
     * @param position   the optional position of the object in the game world
     * @param dimensions the dimensions of the unpickable object
     * @param keyItem    the item required to unlock the object
    ...
    public Unpickable createLockedUnpickable(final String name, final Optional<Point2D> position,
        ...
        return new LockedUnpickable(name, position, dimensions, keyItem, reward);
    }

    /**
     * Creates an unpickable object that does not require unlocking,
     * optionally providing a reward when interacted with.
     *
     * @param name       the name of the unpickable object
     * @param position   the optional position of the object in the game world
     * @param dimensions the dimensions of the unpickable object
